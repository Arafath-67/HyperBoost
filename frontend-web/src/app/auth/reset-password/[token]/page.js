...
export default function ForgotPassword() {
  ...
  const handleSendOtp = async (e) => {
    ...
    try {
      ...
      console.log("OTP Sent (Mock)"); // Testing
      setStatus('idle');
      setStep(2);
    } catch (err) {
      setStatus('error');
  ...
  const handleVerifyOtp = async (e) => {
    ...
    try {
      ...
      // Mock Check
      if(otp.length < 4) throw new Error("Invalid Code");

      setStatus('idle');
      setStep(3); // 🔥 এখন ৩ নম্বর স্টেপে যাবে (Password Reset)
    } catch (err) {
      setStatus('error');
...
// ৩. Reset Password ফাংশন (Frontend)
