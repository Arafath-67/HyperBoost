...
import { 
    Youtube, Play, CheckCircle, Lock, Eye, Clock, Loader2, Ban, 
...
export default function YoutubeTaskPage() {
  ...
  const fetchPlatformTasks = async () => {
    try {
        ...
        if(data.success) {
            // YouTube Specific Filter ('subscribe' & 'view')
            const official = data.tasks.official.find(t => t.platform === TARGET_PLATFORM && t.type === 'subscribe');
            const community = data.tasks.community.find(t => t.platform === TARGET_PLATFORM && t.type === 'subscribe');
  ...
  const openRealTask = (task) => {
      ...
      // Timer Logic (Subscribe = 10s, View = Custom)
      let time = task.timeRequired || 60;
      if(task.type === 'subscribe') time = 10;
  ...
  const handleVerify = async (task) => {
      ...
      try {
          ...
          setIsVerifying(false); 
          alert("Verification Failed. Please subscribe properly."); 
      }
  };

  // --- GATEKEEPER ---
  if (configLoading) return <div className="flex h-[80vh] justify-center items-center"><Loader2 className="animate-spin text-red-600" size={50}/></div>;
  ...
  if (settings && settings.platforms && settings.platforms[TARGET_PLATFORM]?.isEnabled === false) {
      return (
          <div className="flex flex-col items-center justify-center min-h-[70vh] text-center p-12 bg-slate-50 dark:bg-slate-900 rounded-[3rem] border border-red-200 dark:border-red-900/30 shadow-xl">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-lg max-w-md">YouTube tasks are currently disabled by the administrator for maintenance.</p>
          </div>
      );
  }
  
  if (loading) return <div className="flex h-[80vh] justify-center items-center"><Loader2 className="animate-spin text-red-600" size={50}/></div>;
  ...
  return (
    <div className="relative space-y-10 pb-32">
        ...
        {showAd && (
            ...
            >
                <div className="bg-black border border-slate-700 p-10 rounded-[2rem] max-w-xl w-full text-center relative overflow-hidden shadow-2xl">
                    <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-red-500 via-orange-500 to-yellow-500 animate-pulse"></div>
                    
                    <div className="mb-8 flex justify-center">
                        <div className="p-4 bg-slate-800 rounded-full">
                            <Tv size={48} className="text-red-400" />
                        </div>
                    </div>
                    ...
                    <div className="w-full aspect-video bg-slate-800 rounded-2xl border border-slate-700 mb-8 flex flex-col items-center justify-center relative overflow-hidden group">
                        <div className="absolute inset-0 bg-gradient-to-br from-red-900/20 to-orange-900/20 opacity-50"></div>
      ...
      {/* ==============================
          🔥 HEADER SECTION (Premium Red UI)
      ============================== */}
      <div className="flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-red-50 to-white dark:from-red-900/10 dark:to-slate-900 p-10 rounded-[2.5rem] border border-red-100 dark:border-red-500/20 shadow-lg shadow-red-500/5">
        <div className="flex items-center gap-8">
            <div className="w-24 h-24 bg-gradient-to-br from-red-600 to-orange-600 rounded-3xl shadow-xl shadow-red-600/30 flex items-center justify-center text-white transform -rotate-3 hover:rotate-0 transition-transform duration-300">
                <Youtube size={48} fill="currentColor" />
            </div>
            <div>
                <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">YouTube Exchange</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium text-lg flex items-center gap-2">
                    <TrendingUp size={18} className="text-green-500"/>
                    Subscribe & Watch to earn points.
                </p>
            </div>
        </div>
        
        <div className="mt-8 md:mt-0 flex flex-col gap-3">
            <div className="px-6 py-3 bg-white dark:bg-slate-800 border border-red-200 dark:border-red-500/30 text-red-700 dark:text-red-400 rounded-2xl text-sm font-bold flex items-center gap-2 shadow-sm">
      ...
      >
        ...
        {taskOne ? (
            <motion.div variants={cardVariants} className={`relative p-2 rounded-[2.5rem] transition-all duration-500 group ${firstTaskDone ? 'bg-green-500/20' : 'bg-gradient-to-b from-red-500 via-red-600 to-orange-600 shadow-2xl shadow-red-500/20'}`}>
                <div className="bg-white dark:bg-[#0B0F19] rounded-[2.2rem] p-8 h-full flex flex-col relative overflow-hidden">
                    
                    {/* Card Header */}
                    <div className="flex justify-between items-start mb-8">
                        <div className="flex items-center gap-5">
                            <div className="w-16 h-16 rounded-2xl bg-red-50 dark:bg-slate-800 flex items-center justify-center border border-red-100 dark:border-slate-700 shadow-inner text-red-600">
                                <Youtube size={32} />
                            </div>
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-white text-xl line-clamp-1">{taskOne.title || "YouTube Channel"}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="px-2 py-0.5 bg-red-100 dark:bg-red-900/30 text-red-700 dark:text-red-400 text-[10px] font-bold rounded uppercase tracking-wider">Subscribe</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {firstTaskDone ? (
                            ...
                            <div className="text-right">
                                <span className="block text-4xl font-black text-red-600 tracking-tighter">+{taskOne.points}</span>
                    ...
                    {/* Visual Preview Box */}
                    <div className="h-40 bg-slate-50 dark:bg-slate-800/50 rounded-3xl mb-8 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 group-hover:border-red-300 dark:group-hover:border-red-800 transition-colors relative overflow-hidden">
                        <div className="absolute inset-0 bg-red-50/50 dark:bg-red-900/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        
                        {isVerifying && !firstTaskDone ? (
                            <div className="text-red-600 dark:text-red-400 animate-pulse flex flex-col items-center gap-3 font-bold z-10">
                                <Clock size={32}/> 
                            ...
                            <div className="flex flex-col items-center gap-2 z-10 group-hover:scale-110 transition-transform duration-300">
                                <ExternalLink size={28} className="text-slate-400 dark:text-slate-500 group-hover:text-red-600 dark:group-hover:text-red-400"/>
                                <p className="text-slate-400 dark:text-slate-500 text-sm font-bold group-hover:text-red-600 dark:group-hover:text-red-400">Tap to Visit Channel</p>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="mt-auto">
                        {firstTaskDone ? (
                            ...
                            <button 
                                ...
                                className="w-full py-4 bg-slate-900 dark:bg-white text-white dark:text-slate-900 hover:bg-red-600 dark:hover:bg-red-500 dark:hover:text-white font-bold text-lg rounded-2xl transition-all shadow-xl hover:shadow-red-600/30 flex items-center justify-center gap-3 active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
                            >
                                {isVerifying ? "Processing..." : <><Play size={22} fill="currentColor"/> Start Task</>}
                            </button>
                        )}
                    </div>
                </div>
            ...
            <div className="col-span-full py-24 text-center text-slate-400 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                ...
                <button onClick={fetchPlatformTasks} className="mt-4 text-sm text-red-600 font-bold hover:underline flex items-center justify-center gap-1 mx-auto">
                    Refresh List
                </button>
            </div>
        )}
                ...
                <div className="p-8 h-full flex flex-col">
                    <div className="flex justify-between items-start mb-8">
                        <div className="flex items-center gap-4">
                            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                                <Youtube className="text-slate-400 dark:text-slate-500" size={32} />
                            </div>
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-white text-xl line-clamp-1">{taskTwo.title || "User Channel"}</h3>
                    ...
                    <div className="mt-auto">
                        {processingId === taskTwo._id ? (
                            <button onClick={() => handleVerify(taskTwo)} disabled={taskTimer > 0} className="w-full py-4 bg-red-600 text-white font-bold rounded-2xl flex items-center justify-center gap-2 shadow-lg animate-pulse">
      ...
      <div className="pt-16 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {viewTasks.length > 0 ? viewTasks.map((task, i) => (
                <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-red-500/30 transition-all shadow-sm hover:shadow-xl flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-red-50 dark:bg-red-900/10 text-red-600 flex items-center justify-center group-hover:bg-red-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                    ...
                    {processingId === task._id ? (
                        ...
                        <button onClick={() => startAdFlow(task)} className="px-6 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-red-600 dark:hover:bg-red-600 text-slate-700 dark:text-slate-300 hover:text-white text-sm font-bold rounded-xl transition-all">
                            Start
                        </button>
                    )}
                </div>
