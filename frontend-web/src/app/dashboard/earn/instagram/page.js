...
import { 
    Instagram, Heart, CheckCircle, Lock, Eye, Clock, Loader2, Ban, 
    Tv, ExternalLink, Zap, AlertTriangle, ShieldCheck, 
    TrendingUp, Info, Play 
} from 'lucide-react';
import api from '../../../../services/api'; 
import useSettings from '../../../../hooks/useSettings'; 

const TARGET_PLATFORM = 'instagram'; 
...
export default function InstagramTaskPage() {
  ...
  const fetchPlatformTasks = async () => {
    try {
        ...
        if(data.success) {
            // Instagram Specific Filter ('follow' & 'view')
  ...
  const handleVerify = async (task) => {
      ...
      try {
          ...
          setIsVerifying(false); 
          alert("Verification Failed. Please follow the profile properly."); 
      }
  };

  // --- GATEKEEPER ---
  if (configLoading) return <div className="flex h-[80vh] justify-center items-center"><Loader2 className="animate-spin text-pink-600" size={50}/></div>;
  ...
  if (settings && settings.platforms && settings.platforms[TARGET_PLATFORM]?.isEnabled === false) {
      return (
          <div className="flex flex-col items-center justify-center min-h-[70vh] text-center p-12 bg-slate-50 dark:bg-slate-900 rounded-[3rem] border border-red-200 dark:border-red-900/30 shadow-xl">
              ...
              <p className="text-slate-500 dark:text-slate-400 text-lg max-w-md">Instagram tasks are currently disabled by the administrator for maintenance.</p>
          </div>
      );
  }
  
  if (loading) return <div className="flex h-[80vh] justify-center items-center"><Loader2 className="animate-spin text-pink-600" size={50}/></div>;
  ...
  return (
    <div className="relative space-y-10 pb-32">
        ...
        {showAd && (
            ...
            >
                <div className="bg-black border border-slate-700 p-10 rounded-[2rem] max-w-xl w-full text-center relative overflow-hidden shadow-2xl">
                    <div className="absolute top-0 left-0 w-full h-1.5 bg-gradient-to-r from-pink-500 via-purple-500 to-orange-500 animate-pulse"></div>
                    
                    <div className="mb-8 flex justify-center">
                        <div className="p-4 bg-slate-800 rounded-full">
                            <Tv size={48} className="text-pink-400" />
                        </div>
                    </div>
                    ...
                    <div className="w-full aspect-video bg-slate-800 rounded-2xl border border-slate-700 mb-8 flex flex-col items-center justify-center relative overflow-hidden group">
                        <div className="absolute inset-0 bg-gradient-to-br from-pink-900/20 to-purple-900/20 opacity-50"></div>
      ...
      {/* ==============================
          🔥 HEADER SECTION (Premium Pink/Gradient UI)
      ============================== */}
      <div className="flex flex-col md:flex-row justify-between items-center bg-gradient-to-r from-pink-50 to-white dark:from-pink-900/10 dark:to-slate-900 p-10 rounded-[2.5rem] border border-pink-100 dark:border-pink-500/20 shadow-lg shadow-pink-500/5">
        <div className="flex items-center gap-8">
            <div className="w-24 h-24 bg-gradient-to-br from-pink-500 via-purple-500 to-orange-500 rounded-3xl shadow-xl shadow-pink-600/30 flex items-center justify-center text-white transform rotate-3 hover:rotate-0 transition-transform duration-300">
                <Instagram size={48} fill="currentColor" />
            </div>
            <div>
                <h1 className="text-4xl font-extrabold text-slate-900 dark:text-white tracking-tight">Instagram Exchange</h1>
                <p className="text-slate-500 dark:text-slate-400 mt-2 font-medium text-lg flex items-center gap-2">
                    <TrendingUp size={18} className="text-green-500"/>
                    Follow profiles & Heart posts.
                </p>
            </div>
        </div>
        
        <div className="mt-8 md:mt-0 flex flex-col gap-3">
            <div className="px-6 py-3 bg-white dark:bg-slate-800 border border-pink-200 dark:border-pink-500/30 text-pink-700 dark:text-pink-400 rounded-2xl text-sm font-bold flex items-center gap-2 shadow-sm">
      ...
      >
        ...
        {taskOne ? (
            <motion.div variants={cardVariants} className={`relative p-2 rounded-[2.5rem] transition-all duration-500 group ${firstTaskDone ? 'bg-green-500/20' : 'bg-gradient-to-tr from-yellow-400 via-pink-500 to-purple-600 shadow-2xl shadow-pink-500/20'}`}>
                <div className="bg-white dark:bg-[#0B0F19] rounded-[2.2rem] p-8 h-full flex flex-col relative overflow-hidden">
                    
                    {/* Card Header */}
                    <div className="flex justify-between items-start mb-8">
                        <div className="flex items-center gap-5">
                            <div className="w-16 h-16 rounded-2xl bg-pink-50 dark:bg-slate-800 flex items-center justify-center border border-pink-100 dark:border-slate-700 shadow-inner text-pink-600">
                                <Instagram size={32} />
                            </div>
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-white text-xl line-clamp-1">{taskOne.title || "Instagram Profile"}</h3>
                                <div className="flex items-center gap-2 mt-1">
                                    <span className="px-2 py-0.5 bg-pink-100 dark:bg-pink-900/30 text-pink-700 dark:text-pink-400 text-[10px] font-bold rounded uppercase tracking-wider">Follow</span>
                                </div>
                            </div>
                        </div>
                        ...
                        {firstTaskDone ? (
                            ...
                            <div className="text-right">
                                <span className="block text-4xl font-black text-pink-600 tracking-tighter">+{taskOne.points}</span>
                    ...
                    {/* Visual Preview Box */}
                    <div className="h-40 bg-slate-50 dark:bg-slate-800/50 rounded-3xl mb-8 flex flex-col items-center justify-center border-2 border-dashed border-slate-200 dark:border-slate-700 group-hover:border-pink-300 dark:group-hover:border-pink-800 transition-colors relative overflow-hidden">
                        <div className="absolute inset-0 bg-pink-50/50 dark:bg-pink-900/10 opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
                        
                        {isVerifying && !firstTaskDone ? (
                            <div className="text-pink-600 dark:text-pink-400 animate-pulse flex flex-col items-center gap-3 font-bold z-10">
                                <Clock size={32}/> 
                            ...
                            <div className="flex flex-col items-center gap-2 z-10 group-hover:scale-110 transition-transform duration-300">
                                <ExternalLink size={28} className="text-slate-400 dark:text-slate-500 group-hover:text-pink-600 dark:group-hover:text-pink-400"/>
                                <p className="text-slate-400 dark:text-slate-500 text-sm font-bold group-hover:text-pink-600 dark:group-hover:text-pink-400">Tap to Visit Profile</p>
                            </div>
                        )}
                    </div>
            ...
            <div className="col-span-full py-24 text-center text-slate-400 bg-slate-50 dark:bg-slate-900/50 rounded-[2.5rem] border-2 border-dashed border-slate-200 dark:border-slate-800">
                ...
                <button onClick={fetchPlatformTasks} className="mt-4 text-sm text-pink-600 font-bold hover:underline flex items-center justify-center gap-1 mx-auto">
                    Refresh List
                </button>
            </div>
        )}
                ...
                <div className="p-8 h-full flex flex-col">
                    <div className="flex justify-between items-start mb-8">
                        <div className="flex items-center gap-4">
                            <div className="w-16 h-16 rounded-2xl bg-slate-100 dark:bg-slate-800 flex items-center justify-center">
                                <Instagram className="text-slate-400 dark:text-slate-500" size={32} />
                            </div>
                            <div>
                                <h3 className="font-bold text-slate-900 dark:text-white text-xl line-clamp-1">{taskTwo.title || "User Profile"}</h3>
                    ...
                    <div className="mt-auto">
                        {processingId === taskTwo._id ? (
                            <button onClick={() => handleVerify(taskTwo)} disabled={taskTimer > 0} className="w-full py-4 bg-pink-600 text-white font-bold rounded-2xl flex items-center justify-center gap-2 shadow-lg animate-pulse">
      ...
      <div className="pt-16 border-t border-slate-200 dark:border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {viewTasks.length > 0 ? viewTasks.map((task, i) => (
                <div key={i} className="bg-white dark:bg-slate-900 p-6 rounded-2xl border border-slate-200 dark:border-slate-700 hover:border-pink-500/30 transition-all shadow-sm hover:shadow-xl flex items-center justify-between group">
                    <div className="flex items-center gap-4">
                        <div className="w-14 h-14 rounded-2xl bg-pink-50 dark:bg-pink-900/10 text-pink-600 flex items-center justify-center group-hover:bg-pink-600 group-hover:text-white transition-colors duration-300 shadow-sm">
                            <Play size={24} fill="currentColor"/>
                        </div>
                        <div>
                            <p className="font-bold text-lg text-slate-900 dark:text-white line-clamp-1">Watch Reel</p>
                            <div className="flex items-center gap-2 mt-1">
                    ...
                    {processingId === task._id ? (
                        ...
                        <button onClick={() => startAdFlow(task)} className="px-6 py-3 bg-slate-100 dark:bg-slate-800 hover:bg-pink-600 dark:hover:bg-pink-600 text-slate-700 dark:text-slate-300 hover:text-white text-sm font-bold rounded-xl transition-all">
                            Start
                        </button>
                    )}
                </div>
                ...
                <div className="col-span-full py-20 text-center text-slate-400 bg-slate-50/50 dark:bg-slate-900/30 rounded-3xl border-2 border-dashed border-slate-200 dark:border-slate-800">
                    ...
                    <p className="text-lg">No promotional reels available.</p>
                </div>
            )}
        </div>
      </div>
